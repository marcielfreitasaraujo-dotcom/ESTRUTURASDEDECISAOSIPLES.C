...
export function ControlCenter({ initial }: { initial: ControlCenterSnapshot }) {
  ...
  return (
    <div className="grid gap-6">
      <PageHeader
        title="Visão geral"
        description={`${data.greeting} Acompanhe a operação da ${data.storeName}.`}
        actions={
          <>
            <Button asChild>
            ...
            <Button asChild variant="outline">
              <Link href="/app/caixas">Caixas</Link>
            </Button>
          </>
        }
      />
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Vendas hoje" value={formatBRL(data.kpis.todaySalesCents)} hint={`${delta >= 0 ? "+" : ""}${delta}% em relação a ontem`} />
        ...
        <StatCard
          ...
          tone={data.cash.open ? "success" : undefined}
        />
      </section>
        ...
        <Card>
          ...
          <CardContent className="grid gap-2">
            ...
            {data.recentOrders.map((order) => (
              <Link key={order.id} href="/app/pedidos" className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border px-3 py-2 text-sm hover:bg-muted/40">
        ...
        <Card>
          ...
          <CardContent className="grid gap-2">
            ...
            {data.alerts.map((alert) => (
              <Link key={alert.id} href={alert.href} className="rounded-lg border border-border px-3 py-2 text-sm hover:bg-muted/40">
...
function Block({ title, href, lines }: { title: string; href: string; lines: string[] }) {
  return (
    <Link href={href} className="rounded-lg border border-border p-3 hover:bg-muted/30">
