  ...
  return (
    <div className="grid gap-4">
      <div>
        ...
        <p className="text-sm text-zinc-400">Linha do tempo deste turno. Nada é apagado: cancelamentos ficam com status.</p>
      </div>
      <ol className="grid gap-2">
        {rows.map((row) => {
          const outbound = row.type === "SANGRIA" || row.type === "EXPENSE" || row.type === "REFUND";
          return (
            <li key={row.id} className="rounded-xl border border-zinc-800 bg-card px-4 py-3">
