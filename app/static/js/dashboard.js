(() => {
  ...
  const registrar = window.finupRegistrarGrafico || ((c) => c);
  ...
  const formatarMoeda = (valor) => {
    ...
    const textoMoeda = `R$ ${corpo},${frac}`;
    return neg ? `- ${textoMoeda}` : textoMoeda;
  };
    ...
    registrar(
      new Chart(evo, {
        ...
        options: {
          ...
          scales: {
            x: { grid: { display: false }, ticks: { color: muted, maxRotation: 0, autoSkip: true, maxTicksLimit: 10 } },
